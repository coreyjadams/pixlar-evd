...
namespace pixevd {
  ...
  const pixevd::VoxelSet& VoxelSetArray::voxel_set(InstanceID_t id) const
  {
  ...
  void VoxelSetArray::emplace(std::vector<pixevd::VoxelSet>&& voxel_vv)
  { 
  ...
  inline void VoxelSetArray::insert(const std::vector<pixevd::VoxelSet>& voxel_vv)
  { 
    _voxel_vv = voxel_vv;
    for(size_t id=0; id<_voxel_vv.size(); ++id)
      _voxel_vv[id].id(id);
  ...
  void VoxelSetArray::emplace(pixevd::VoxelSet&& voxel_v)
  {
  ...
  void VoxelSetArray::insert(const pixevd::VoxelSet& voxel_v)
  {
    if(voxel_v.id() >= _voxel_vv.size()) {
      size_t orig_size = _voxel_vv.size();
      _voxel_vv.resize(voxel_v.id()+1);
  ...
  pixevd::VoxelSet& VoxelSetArray::writeable_voxel_set(const InstanceID_t id)
  {
    if(id >= _voxel_vv.size()) {
      std::cerr << "VoxelSetArray has no VoxelSet with InstanceID_t " << id << std::endl;
      throw std::exception();
